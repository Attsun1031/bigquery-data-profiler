...
from bqdataprofiler.models.error import Error, ErrorCollection
...
from bqdataprofiler.querybuilder import BigQueryProfileQueryBuilder, python_type_to_bigquery_type
...
def run_profile(root_setting: ProfileSettingRoot, result: ProfilingResult, dry_run: bool = False):
    ...
    for project_setting in root_setting.settings:
        ...
        with ThreadPoolExecutor(max_workers=root_setting.max_workers) as executor:
            _execute_profiling(executor, dry_run, errors, query_builder, result, root_setting, tables)
...
def _execute_profiling(executor, dry_run, errors, query_builder, result, root_setting, tables):
    future_to_table = {}
    for tbl in tables:
...
def _profile_table(tbl: bigquery.Table, query_builder: BigQueryProfileQueryBuilder, maximum_bytes_billed: int,
    ...
    rows = list(result)
    if not rows:
        logger.warning(f"Cannot get metrics for table: {tbl.full_table_id}, query: {query}")
        return [], total_bytes_processed

    def _to_metric(_name: str, _value: Any) -> Metric:
        metric_name, col_name = query_builder.column_name_to_metric_target(_name)
